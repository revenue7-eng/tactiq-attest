...
Amended: 25 Sep 2026, before implementation (decisions 2 and 7, boundary E,
the secret on the console, the public-only negative case). The protocol of
...
2. **The EK is the RSA-2048 EK whose certificate is at `0x01C00002`,**
   ...
   verified. The registrar makes the credential blob from the public key in
   the certificate, never from a key the device reports. The blob opens only
   in a TPM that holds the private half of that key, so a successful
   activation is itself the proof that the certificate describes this TPM's
   EK, and a TPM whose EK differs cannot complete registration. No separate
   comparison of the recreated EK with the certificate is part of the
   protocol; on the bench it is a diagnostic for a failed activation (done
   once, see Verification).
...
7. **Registration is checked outside gate 1.** `attest-appraise` stays as it
   ...
   verifies a registration record (EK chain, AK rule, AK name recomputed
   from the AK public area; the blob itself can be checked only by
   activating it on the device, boundary A) and emits the AK
...
E. The protocol relies on the TPM refusing `TPM2_ActivateCredential` for an
...
**Public-only object, swtpm** (0.7.3, tpm2-tools 5.6). The public key of a
P-256 key generated by OpenSSL was loaded with `tpm2_loadexternal` with the
attributes `fixedtpm|fixedparent|sensitivedataorigin|userwithauth|restricted|sign`
and `ecc:ecdsa-sha256:null`, which satisfy the DDR-004 decision 4 rule. The
TPM accepted every load. A blob was made for each resulting name to the EK
public area written by `tpm2_createek` (on swtpm it is the certificate's
key), and `tpm2_activatecredential` was refused with
`TPM_RC_AUTH_UNAVAILABLE` (`0x12F`) in all four variants: empty `authPolicy`
in the null hierarchy; `authPolicy` set to PolicyCommandCode(ActivateCredential)
and satisfied by a policy session; the same with `adminwithpolicy`; the same
loaded in the endorsement hierarchy. A control run with an AK created by
`tpm2_createak` under the same EK released the secret. The TPM grants no
authorization (neither authValue nor policy) to an object without its
sensitive part, so the ADMIN role that activation requires is unreachable
(boundary E).

The secret on the console. `tpm2_activatecredential` prints the released
secret (`certinfodata:`). The protocol does not rely on the secret staying
confidential after the TPM has released it: the secret is fresh for each
registration, the blob is bound to the AK name and to the EK, and whoever
reads the secret learns only what the TPM has already shown. A copy of it
does not let anyone register another key. The bench registration may
therefore carry the secret back through the console. The record still
